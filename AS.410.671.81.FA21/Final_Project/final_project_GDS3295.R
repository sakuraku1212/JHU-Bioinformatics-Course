...
library(GEOquery)
dataset <- getGEO("GDS3295", GSEMatrix =TRUE, getGPL=FALSE)
...
colnames(dat)<-paste(sample,gender)
...
pv<- apply(dat.log2,1,t.test.all.genes,s1=gender=='female',s2=gender=='male')
...
female.m <- apply(dat.log2 [,female],1,mean,na.rm=T)
male.m <- apply(dat.log2 [,male],1,mean,na.rm=T)
...
ex <- na.omit(ex) # eliminate rows with NAs
ex <- ex[!duplicated(ex), ]  # remove duplicates
ex<-as.data.frame(ex2)
ex_clean <- ex2[,-c(2, 5, 17, 21, 44, 49, 59, 62, 102)]
is.data.frame(ex2)
...
ex_clean<-as.data.frame(ex_clean)
