...
library(GEOquery)
dataset <- getGEO("GDS1210", GSEMatrix =TRUE, getGPL=FALSE)
...
corrplot(cor(cor_dat), method = "color",  mar=c(2,0,3,0),
         title =" correlation matrix of Gastric cancer samples and normal samples")
...
plot(dat.clust,labels=names(dat),cex=0.75, 
     main="Hierarchical Clustering Dendrogram of Gastric cancer samples and normal samples")
...
plot(dat.mean,dat.cv,main="Gastric cancer samples and normal samples CV vs. Mean", 
...
plot(c(1,length(dat.avg)),range(dat.avg),type="n",xlab="",ylab="Avg r",
     main="Avg correlation of Gastric cancer samples and normal samples",axes=F)
...
##Remove the outliers: GSM51770 normal
dat_clean <- dat[,-c(8)]
...
pv<- apply(dat_clean,1,t.test.all.genes,s1=factor=='normal',s2=factor=='cancer')
...
top_gene<-subset(adj.p.bonferroni,adj.p.bonferroni<=0.05)  #193
length((top_gene))
...
normal<-factor[1:7]
cancer<-factor[8:29]
normal.m <- apply(dat.log2 [,normal],1,mean,na.rm=T)
cancer.m <- apply(dat.log2 [,cancer],1,mean,na.rm=T)
fold <- normal.m- cancer.m
summary(fold)

fold_abs<-abs(2^fold)
probesets<-subset(dat_clean, adj.p.bonferroni<=0.05 & fold_abs>2)
length(probesets) #29
row.names(probesets)
