...
library(GEOquery)
dataset <- getGEO("GDS3622", GSEMatrix =TRUE, getGPL=FALSE)
...
genotype<-dataset@dataTable@columns[["genotype/variation"]]
colnames(dat)<-paste(sample,genotype)
...
dat_clean<-dat[m>quantile(m, probs=.10),]
...
pv<- apply(dat_clean,1,t.test.all.genes,s1=genotype=='wild type',s2=genotype=='Nrf2 null')
...
wildtype<-genotype[1:56]
Nrf2null<-genotype[57:110]

wildtype.m <- apply(dat.log2 [,wildtype],1,mean,na.rm=T)
Nrf2null.m <- apply(dat.log2 [,Nrf2null],1,mean,na.rm=T)
fold <- wildtype.m - Nrf2null.m
summary(fold)
